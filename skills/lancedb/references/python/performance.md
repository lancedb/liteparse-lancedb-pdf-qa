...
Add maintenance code only for embedded OSS/local tables. Do not add this to LanceDB Enterprise/Cloud remote table code; remote compaction and cleanup are handled automatically based on the Enterprise cluster configuration.
...
For local/OSS tables, run `optimize()` after large writes, after many update/delete operations, or on a regular maintenance schedule:
